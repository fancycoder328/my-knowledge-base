    ...
    <AnimatePresence mode="wait">
      ...
      >
        {navItems.map((navItem: any, idx: number) => (
          <span
            key={`link=${idx}`}
            onClick={() => window.open(navItem.link, `${navItem.target}`)}
            className={cn(
        ...
        <button onClick={handleClick} className="border text-sm font-medium relative border-neutral-200 dark:border-white/[0.2] text-black dark:text-white px-4 py-2 rounded-full">
          <span>Repack</span>
